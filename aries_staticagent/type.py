...
class Type:
    ...
    def __init__(
        self, doc_uri: str, protocol: str, version: Union[str, Semver], name: str
    ):
        if isinstance(version, str):
            try:
                self.version_info = Semver.from_str(version)
            except ValueError as err:
                raise InvalidType("Invalid type version {}".format(version)) from err
        ...
        self._normalized = Type.FORMAT.format(
            self.doc_uri, self.protocol, self.version_info, self.name
        )
