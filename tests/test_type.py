...
@pytest.mark.parametrize("version_arg", ["a.b.c", 10])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("doc;protocol/1.0.0-alpha/name", ["doc;", "protocol", "1.0.0-alpha", "name"]),
    ],
)
def test_parse_from_str(type_str, expected):
    """Test parsing type from string."""
    type_ = Type.from_str(type_str)
    assert [type_.doc_uri, type_.protocol, type_.version, type_.name] == expected
